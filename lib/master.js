...
const getPromiseHandler = (func) => {
  ...
  if ('mGet' === func) {
    ...
    handler = setMultipleValues;
  } else if ('mDel' === func) {
    handler = deleteMultipleValues;
  } else if (['decr', 'incr'].includes(func)) {
    handler = incrementOrDecrement;
  } else if (['max', 'maxAge', 'stale', 'itemCount', 'length'].includes(func)) {
    handler = getCacheConfigValue;
  } else {
    handler = defaultLruFunction;
  }
  return handler;
};
...
const getPromisified = (namespace, options) => {
  return (...args) => {
    ...
    const promise = getPromiseHandler(func)(namespace, options, func, funcArgs);
    return promise;
  };
};
...
const getMessageHandler = (func) => {
  let handler;
  if ('()' === func) {
    handler = handleConstructLruCache;
  } else if ('mGet' === func) {
    handler = handleGetMultipleValues;
  } else if ('mSet' === func) {
    handler = handleSetMultipleValues;
  } else if ('mDel' === func) {
    handler = handleDeleteMultipleValues;
  } else if (['decr', 'incr'].includes(func)) {
    handler = handleIncrementOrDecrement;
  } else if (['max', 'maxAge', 'stale', 'itemCount', 'length'].includes(func)) {
    handler = handleGetCacheConfigValue;
  } else {
    handler = handleDefaultLruFunction;
  }
  return handler;
};
...
if (cluster.isMaster) {
  ...
  cluster.on('fork', (worker) => {
    ...
    worker.on('message', (request) => {
      ...
      return getMessageHandler(request.func)(request, worker);
    });
  });
}
