...
const promisifiedGetCacheConfigValue = (lru, func, funcArgs) =>
  new Promise((resolve) => {
    if (funcArgs[0]) {
      lru[func] = funcArgs[0];
    }
...
const promisifiedIncrementOrDecrement = (lru, func, funcArgs) =>
  new Promise((resolve) => {
...
const promisifiedGetMultipleValues = (lru, func, funcArgs) =>
  new Promise((resolve) => {
    const mGetValues = utils.mGet(lru, funcArgs);
    return resolve(mGetValues);
  });

const promisifiedSetMultipleValues = (lru, func, funcArgs) =>
  new Promise((resolve) => {
    utils.mSet(lru, funcArgs);
    return resolve(true);
  });

const promisifiedDeleteMultipleValues = (lru, func, funcArgs) =>
  new Promise((resolve) => {
    utils.mDel(lru, funcArgs);
    return resolve(true);
  });

const promisifiedDefaultLruFunc = (lru, func, funcArgs) =>
  new Promise((resolve) => {
...
const getPromisified = (namespace, options) => {
  ...
  return (...args) => {
    ...
    switch (func) {
      ...
      case 'itemCount':
      case 'length': {
        return promisifiedGetCacheConfigValue(lru, func, funcArgs);
      }
      case 'decr':
      case 'incr': {
        return promisifiedIncrementOrDecrement(lru, func, funcArgs);
      }
      case 'mGet': {
        return promisifiedGetMultipleValues(lru, func, funcArgs);
      }
      case 'mSet': {
        return promisifiedSetMultipleValues(lru, func, funcArgs);
      }
      case 'mDel': {
        return promisifiedDeleteMultipleValues(lru, func, funcArgs);
      }
    }
    return promisifiedDefaultLruFunc(lru, func, funcArgs);
  };
};
